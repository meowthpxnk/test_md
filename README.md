...
Основная задача приложения заключена в мэнеджменте сообщений из WhatsApp до выделенного сервера и от выделенного сервера в WhatsApp.

### Обработка сообщений
У обоих серверов есть свой обработчик сообщений. При получении сообщения обработчик преобразовывает их в валидный формат для конечной точки и кладет в очередь отправки сообщений. У каждого обработчика своя очередь отправки.

Очередь создана для снижения нагрузки на сервер при большом потоке сообщений.
...
1. Из директории проекта выполняем команду
```sh
tsc
```
...
* Входящие сообщения - сообщения, отправляемые из мессенджера в Root servr;
* Исходящие сообщения - сообщения, отправляемые из Root server в мессенджер;